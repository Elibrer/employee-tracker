...
const {ViewAllDepartments, ViewAllRoles, ViewAllEmployees, AddDepartment, AddRole, ViewManagers, AddEmployee, UpdateEmployeeRole, UpdateEmployeeManager, DeleteQuery} = require('./queries.js');
    ...
    const { manager } = await inquirer.prompt(
        {
            ...
            choices: managersTable.map(manager => manager.manager)
        }
    );
    const viewEmployeesByManager = new ViewEmployeesByManager(localHostData, manager);
    const employeesTable = await viewEmployeesByManager.query();
    return employeesTable;
}
