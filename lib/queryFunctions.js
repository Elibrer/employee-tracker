...
const viewEmployeesByManager = async (localHostData) => {
    ...
    let validManagerID = false;
    for (let i = 0; i < managersTable.length; i++) {
        if (managersTable[i]['Manager Id'] === parseInt(managerID)) {
            validManagerID = true;
            break;
        }
    }
...
const viewCombinedSalaries = async (localHostData) => {
    const departmentsTable = await viewAllDepartments(localHostData);
    console.table(departmentsTable);

    let departments = departmentsTable.map(department => department['Department Name'])
