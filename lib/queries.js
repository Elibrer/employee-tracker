...
class ViewAllRoles extends DbAccess {
    constructor(hostData) {
        super(hostData);
        this.sql = `SELECT title AS \`Role Title\`, role.id AS \`Role Id\`, department.name AS Department, CONCAT('$', FORMAT(salary, 2))
        FROM role
...
class ViewManagers extends DbAccess {
    constructor(hostData) {
        super(hostData);
        this.sql = `SELECT CONCAT(employee.first_name, ' ', employee.last_name) AS \`Manager Name\`, role.title AS \`Role Title\`, employee.id AS \`Manager Id\`, CONCAT('$', FORMAT(salary, 2)) AS \`Salary\`
        FROM employee employee
        JOIN role ON employee.role_id = role.id
